...
#Majority of code from https://github.com/jhonatanoliveira/dcspns
...
class SumProductNetwork:
    ...
    def __init__(self, input_shape, seed=1234, name="SPN", reuseValue=0):
        """
    ...
    def build_forward(self, forward_input, reuse=False, external_weights=None):
        """
        ...
        for curr_layer in rev_top_sort:
            ...
            #Create an empty weight slice
            weight_slice = None

            #if the current layer is a sum layer(and therefore has weights) and there are external weights to load 
            if isinstance(curr_layer, SumLayer) and external_weights!=None:
                #Go to the function to split the weight and get back the reshaped weight slice and then new starting position for the next weight slice
                start_pos, weight_slice = splitWeight(external_weights, children_output_shape, curr_layer, start_pos)

            #Go to the build function of the current layer
            curr_layer.build(children_output_shape, external_tensor=weight_slice, reuse=reuse)
        ...
        print("\n\nBefore attempted assignment")

        #This is to keep track of where in the external tensor to start the particular weight slice
        start_pos=0
        rev_top_sort = reversed(kahn_topsort(self.layers_graph["children"]))
        for curr_layer in rev_top_sort:
            print(curr_layer)
            ...
            if isinstance(curr_layer, SumLayer) and external_weights!=None:
                ...
                print("In here")
                print("Old data: ", curr_layer)
        
                #Go to the function to split the weight and get back the reshaped weight slice and then new starting position for the next weight slice
                start_pos, weight_slice = splitWeight(external_weights, curr_layer.input_shape, curr_layer, start_pos)
                print(weight_slice)
        
                #Go to the build function of the current layer
                #curr_layer.build(children_output_shape, external_tensor=weight_slice, reuse=reuse)
                curr_layer.weights = weight_slice
                print("New data: ", curr_layer)
        ...
        print("After attempted assignment\n\n")
    ...
    def build_backward_masks(self, forward, sampling_amt=None):
        ...
        #so the command executed is tf.shape(forward[self.root_layer]["output"])[0]

        print(tf.shape(forward[self.root_layer]["output"])[0])
        ...
        print("top_sort: ", top_sort) #A list of the layers
        ...
        for curr_layer in top_sort:
            ...
            if curr_layer not in self.layers_graph["parents"] or\
                ...
                parents_masks = backward_masks[
                    None]["output_mask"]
                print("parents_mask: ", parents_masks)
    ...
    def compile(self, optimizer="adam", learning_rate=0.001, reuse= False, external_weights=None):
        """
        ...
        """
        # Loss function
        forward = self.build_forward(self.inputs, reuse = reuse, external_weights=external_weights)
    ...
    def fit(self, train_data, train_labels=None, epochs=50, minibatch_size=64,
        ...
        # Settings
        num_instances = train_data.shape[0] #the number of instances is correct
        ...
        for epoch in range(epochs):
            ...
            print("Fitting: running epoch {}".format(epoch))
            ...
            #The alternate for statement and minibatch statement are to speed up testing as the code takes a very long time to run
...
#A function for splitting the imported tensor into the correct size weight for each layer
def splitWeight(external_weights, input_shape, curr_layer, start_pos = 0):

    #Gets the shape of the weight
    _var_shape = [input_shape[0], input_shape[1],
                input_shape[2], curr_layer.out_channels]


    #gets the size of the weight
    size_of_weight = _var_shape[0]*_var_shape[1]*_var_shape[2]*_var_shape[3]

    #Gets the end position of the weight for slicing
    end_pos = start_pos+size_of_weight

    #SLice thhe weight
    weight_slice= external_weights[start_pos:end_pos]

    #Reshape the weight so that it is the correct shape for the DCSPN
    weight_slice = tf.reshape(weight_slice, _var_shape)


    #This multiplication step is because the values were so small they were being evaluated at zero for the NLL
    #It is removable but I left it in to show that the weights are actually loading
    weight_slice= tf.math.scalar_mul(10000, weight_slice)


    #return the ending positon of the imported tensor and the newly created weight tensor
    return end_pos, weight_slice
