...
class Layer:
    ...
    def build(self, input_shape, external_tensor = None, reuse=False):
        """
...
class SumLayer(Layer):
    ...
    def build(self, input_shape, external_tensor = None, reuse=False):
        ...
        #If there is an external tensor given
        if (external_tensor != None):
            #Still using the sum layer
            with tf.variable_scope("SumLayer", reuse=False):
                #Connect the two graphs
                self.weights = external_tensor
