...
def define_SPN():
    ...
    forward = spn_model.compile(learning_rate=0.01,
                        optimizer="adam",
                        reuse=False,
                        external_weights=None) #This will be the tensor that is sliced and reshaped
    ...
    #for each layer
    for layer in spn_model.layers:
        #Test if the layer is a sum layer
        if isinstance(layer, SumLayer):
            
            hold_shape = layer.weights.get_shape().as_list()    #get the shape of the weight as a list
            spn_struct.append(hold_shape)   #append it to the array
...
# TODO: create new function called get_spn_layer_sizes(spn)
# This function contains code from Landmark-1
# Remove code from Landmark-2, that is no "compile" or "build_forward"
# Therefore, we need to traverse the DCSPN graph using "lasyers_graph" with either "children" or "parents"
# For each SumLayer, compute the weights shape using the previous (parent) layer size and the "out_channel" size of this current SumLayer. Take a look at "SumLayer.build" and "calling build in build_forward".
...
#TODO: No new SPN, use the previous one built in "build_spn"


#From here down is the actually trainable DCSPN that loads the weights
initializer = "uniform" #for the layer parameters
...
spn.fit(train_data=train_imgs, epochs=0, add_to_feed=feed_means_stds, minibatch_size=64)
