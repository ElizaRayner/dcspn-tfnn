"""
Creates a DCSPN to find the size of the DCSPN weights. Then uses the function to build the NN and attach the weights to a new DCSPN
Runs using libraries from https://github.com/jhonatanoliveira/dcspns
"""
...
from dcspn.layers import SumLayer, ProductLayer, GaussianLeafLayer, LeafLayer
from dcspn.utilities import Database, plot_cost_graph, kahn_topsort
...
def create_nn(spn_struct, nn_struct):
    ...
    nn_layers = []  #For keeping track of each layer
    ...
    for shape in nn_struct:
        ...
        nn_layers.append(previous)  #Append the new layer
    ...
    #Append to keep track of the final layer
    nn_layers.append(final_output)
...
def get_spn_layer_sizes(spn):
    ...
    print(spn.layers_graph["children"])
    ...
    forward = {
        forward_input: {
            "output_shape": spn.input_shape,
            "output": forward_input
        }
    }

    #For each layer, most of this code is modified from the build_forward function
    for curr_layer in spn.layers_graph["children"]:

        children_output = []
        children_output_shape = []
            

        #If the current layer is a leaf layer
        ...
        if isinstance(curr_layer, ProductLayer):
            ...
            for child in spn.layers_graph["children"][curr_layer]:
            ...
            print("children_output_shape: ", children_output_shape)
        ...
        if isinstance(curr_layer, SumLayer):
            ...
            for cidx, child in enumerate(
                    spn.layers_graph["children"][curr_layer]):
                # Add channels
            ...
            #children_output_shape[3] = curr_layer.out_channels
            print("children_output_shape: ", children_output_shape)



            #Append the weight sizes to the array
            children_output_shape = [children_output_shape[0], children_output_shape[1],
                          children_output_shape[2], curr_layer.out_channels]
            print("_var_shape: ", children_output_shape)
            spn_struct.append(children_output_shape)   #append it to the array
            print(spn_struct)
    ...
    return spn_struct   #return the sizes of the weights



#This function creates a test DCSPN 
def define_SPN():
    ...
    spn_model = SumProductNetwork(input_shape=input_shape, reuseValue=reuse)
    ...
    spn_model.add_layer(leaf_layer)#adds the leaf layer to the graph
    spn_model.set_leaf_layer(leaf_layer)#sets it as the leaf layer
    ...
    spn_model.add_forward_layer_edge(leaf_layer, sum_layer)
    ...
    spn_model.add_forward_layer_edge(sum_layer, product_layer)
    ...
    spn_model.add_forward_layer_edge(product_layer, root_layer)
    ...
    spn_model.set_root_layer(root_layer)
    ...
    spn_struct = get_spn_layer_sizes(spn_model)
    return spn_struct, spn_model
...
#get the struct of the DCSPN
spn_struct, spn = define_SPN()
...
#Call the function to create the NN and get the output
external_tensor = create_nn(spn_struct, nn_struct)

print(external_tensor)
